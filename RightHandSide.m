function [ b ] = RightHandSide( node,elem )
...
%   Input node,elem, function, compute the right hand side vector
 ...
 bt1 = area.*(rhsfunction(mid2)+rhsfunction(mid3))/6;
 bt2 = area.*(rhsfunction(mid3)+rhsfunction(mid1))/6;
 bt3 = area.*(rhsfunction(mid1)+rhsfunction(mid2))/6;
